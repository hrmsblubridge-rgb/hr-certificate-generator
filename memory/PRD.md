...
- **v19 (2026-09-05/07)** Doc module follow-ups + designations. (a) **Orphan headings fixed**: keep-with-next now also recognises short all-bold paragraphs such as `10. CONFIDENTIALITY` (Word clause titles), not just `h1-h6`. (b) **Signature block is now equal 50/50 columns** across the full text width (explicit per-cell px widths — mupdf ignores percentage widths), label at 9.5px so long company names stay on one line, and four blank lines of clear space above `Signature:` in both cells for the seal/wet signature (user chose "space, no image"). (c) **Imported DOCX tables keep their authored column proportions**, scaled to the letterhead text width. (d) **New `POST /api/doc/docx`** (`backend/doc_docx.py`) → editable Word file: A4 with margins mirroring the PDF safe area, the letterhead's header and footer bands rasterised straight from the master PDF into the Word section header/footer (so every page carries them), Arial 11pt, headings/lists/tables/alignment/inline formatting preserved, signature block or seal appended. Frontend: "Download DOCX" button beside Download PDF (`data-testid=doc-download-docx`). (e) **Auth bug fixed** (pre-existing): `assert_not_locked` compared a naive MongoDB datetime with an aware one, so `/api/auth/login` returned 500 instead of 429 for any locked-out identifier — `locked_until` is now normalised to UTC. (f) **Two designations added** to all four designation lists (Certificate, Offer Letter, Offer Letter (Email), Offer of Appointment): **AI Business Solutions Executive** and **IT Systems & Infrastructure Administrator** — verified end-to-end through the offer-email preview and the 22-page Offer-of-Appointment PDF (ampersand renders correctly). Doc suites: 35/35 pytest green.
